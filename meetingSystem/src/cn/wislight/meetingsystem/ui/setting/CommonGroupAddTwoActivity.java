...
public class CommonGroupAddTwoActivity extends BaseActivity implements
		OnClickListener {
	...
	public void initView() {
		...
		String strListHasSel = group.getString(Constants.GROUP_MEMBER_LIST, "");
	...
	public void clickBeginSearch(View view){
		...
		if (ctype == 2 || ctype == 1){
			if (orgname.length() == 0){
				Toast.makeText(CommonGroupAddTwoActivity.this, 
	...
	private void searchPersionInfo(int type, String orgname, String pplname) {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler(){
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(CommonGroupAddTwoActivity.this, getString(R.string.error_network), Toast.LENGTH_SHORT).show();
                finish();  
			}
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("用户未登陆")){
					Toast.makeText(CommonGroupAddTwoActivity.this, getString(R.string.error_login), Toast.LENGTH_SHORT).show();
					return;
				}	
				...
				try {
					...
					e.printStackTrace();
					Toast.makeText(CommonGroupAddTwoActivity.this, getString(R.string.error_dataabout), Toast.LENGTH_SHORT).show();
	...
	protected void onPause(){
		...
		if (listHasSel.size() > 0){
			...
			editor.putString(Constants.GROUP_MEMBER_LIST, beanListToJson);
			editor.commit();
		}
	}
    ...
    private void addGroup() {
		// TODO Auto-generated method stub
    	if (listHasSel.size() == 0){
    		Toast.makeText(CommonGroupAddTwoActivity.this, getString(R.string.error_no_group_member_selected), Toast.LENGTH_SHORT).show();
    		return;
    	}
		...
		String url = "MeetingManage/mobile/createGroup.action";
		...
		params.put("groupremark", group_remark);
