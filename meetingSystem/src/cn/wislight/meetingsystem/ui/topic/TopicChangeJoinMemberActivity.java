...
public class TopicChangeJoinMemberActivity extends BaseActivity implements
		OnClickListener {
	...
	public void initView() {
		...
		topicId = getIntent().getStringExtra(Constants.TOPIC_ID);
		topicNo = getIntent().getStringExtra(Constants.TOPIC_NO);	
		meetNoZd = getIntent().getStringExtra(Constants.MEET_NO);	
	...
	private void getTopicAttenders() {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(TopicChangeJoinMemberActivity.this,
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("用户未登陆")){
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_login), Toast.LENGTH_SHORT).show();
					return;
				}	
				
				JSONArray joinmember_list;
				try {
					joinmember_list = new JSONObject(response).getJSONArray("meetTopicList");
	...
	private void searchMyCreatedTempPerson() {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler(){
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_network), Toast.LENGTH_SHORT).show();
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				try {
					...
					e.printStackTrace();
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_dataabout), Toast.LENGTH_SHORT).show();
    ...
    private void deleteMeetPersonTmp() {
    	...
    	if (length == 0){
    			Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_no_selected_attender), Toast.LENGTH_SHORT).show();
    		return;
    	}
		...
		String url="MeetingManage/mobile/deleteAttenderById.action";
		RequestParams params = new RequestParams();
		params.put("meetNoZd", meetNoZd);
		...
		MeetingSystemClient.post(url, params, new AsyncHttpResponseHandler()
		{
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("success")){
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.topic_member_change_success), Toast.LENGTH_SHORT).show();
					getTopicAttenders();
				} else{
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.topic_member_change_fail), Toast.LENGTH_SHORT).show();
				}
			}
		});	
	}
	...
	private void AddToHasSel() {
    	...
    	if (length == 0){
    		if (bDuplicated){
    			Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_person_duplicted), Toast.LENGTH_SHORT).show();
    		}else{
    			Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.error_no_person_to_add), Toast.LENGTH_SHORT).show();	
    		}
    		return;
    	}
		String topic_joinmember = "";     
		...
		String url="MeetingManage/mobile/updateTopicAttender.action";
		RequestParams params = new RequestParams();
		params.put("info", topic_joinmember);
		...
		MeetingSystemClient.post(url, params, new AsyncHttpResponseHandler()
		{
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("success")){
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.conf_member_change_success), Toast.LENGTH_SHORT).show();
					for (Element e: list){
						e.isCheck = false;
					}
					...
					try {
						...
						if (jsonArray.length() > 0){
							...
							IntentUtil.startActivity(TopicChangeJoinMemberActivity.this, AttenderConflictListActivity.class, intent);
							finish();
						}else{
							getTopicAttenders();
							mAdapter.notifyDataSetChanged();
					...
					Toast.makeText(TopicChangeJoinMemberActivity.this, getString(R.string.conf_member_change_fail), Toast.LENGTH_SHORT).show();
				}
			}
		});	
	}
