 ...
 * 主页  新增议题第一个
 */
public class TopicMeetAddOneActivity extends BaseActivity {
	...
	public void initView() {
		...
		etopic = this.getSharedPreferences(Constants.TOPIC_ME_NODE+ Variables.loginname, MODE_PRIVATE);
	...
	private void getTopicDetailedData(String id) {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();  
				Toast.makeText(TopicMeetAddOneActivity.this,
	...
	public void setupView() {
		setContentView(R.layout.topic_me_addone);
	...
	public void  clickNext(View view){
		if (etKeywords.getText().toString().length() == 0){
			Toast.makeText(TopicMeetAddOneActivity.this, getString(R.string.error_no_topic_keys), Toast.LENGTH_SHORT).show();
			return;
		}
		if (etSummary.getText().toString().length() == 0){
			Toast.makeText(TopicMeetAddOneActivity.this, getString(R.string.error_no_topic_summary), Toast.LENGTH_SHORT).show();
			return;
		}
		IntentUtil.startActivity(this,TopicMeetAddTwoActivity.class);
		this.finish();
	}
