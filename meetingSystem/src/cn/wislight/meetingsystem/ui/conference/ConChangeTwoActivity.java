 ...
 *	会议发起
 */
public class ConChangeTwoActivity extends BaseActivity implements OnClickListener{
	...
	public void initView() {
		conference = this.getSharedPreferences("conference_change_"+ Variables.loginname, MODE_PRIVATE);
	...
	public void setupView() {
		setContentView(R.layout.conference_change_two);
	}
	...
	public void onClick(View v) {
		...
		case R.id.conf_btn_up:
			...
			IntentUtil.startActivity(this,ConChangeOneActivity.class, intent);
			finish();
			break;
		...
		case R.id.conf_joinmeb:
			...
			intentc.putExtra(Constants.MEET_NO, meetNo);
			intentc.putExtra(Constants.ID, meetId);
			IntentUtil.startActivityForResult(this, ConfChangeJoinMemberActivity.class, Constants.CODE_CONF_JOINMEMBER, intentc);

			break;	
		case R.id.conf_contromeb:
			Intent intent1 = new Intent();
			intent1.putExtra("type", "0");
			IntentUtil.startActivityForResult(this, ConfSelectApplicatorActivity.class, Constants.CODE_CONF_CONTROLMEMBER);
			break;	
		case R.id.conf_accessmeb:
			Intent intent2 = new Intent();
			intent2.putExtra("type", "1");
			IntentUtil.startActivityForResult(this, ConfSelectApplicatorActivity.class, Constants.CODE_CONF_ACCESSMEMBER);
	...
	public void clickTopicList(View view){
		...
		intent.putExtra(Constants.MEET_NO, meetNo);
		intent.putExtra(Constants.ID, meetId);
		IntentUtil.startActivity(this, ConfChangeTopicListActivity.class, intent);
	}

	private boolean checkTime(){
		boolean result = true;
		String topic_start_date = conference.getString("conference_endtime", "");
        ...
        if (starttime.compareTo(currenttime) <= 0){
        	Toast.makeText(ConChangeTwoActivity.this, "会议结束时间不能小于当前时间", Toast.LENGTH_SHORT).show();
			return false;
        }   
        /*
		...
		if(resultCode == RESULT_OK && requestCode == Constants.CODE_FILE_SELECT_COMMON){
			...
			if(filePath != null){
				...
				if (f.length() > Constants.MAX_UPLOAD_FILE_SIZE){
					Toast.makeText(ConChangeTwoActivity.this,
	...
	public void clickSaveBaseInfo(View view){
		if (conference.getString("conference_controlmember", "").length() == 0){
			Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_no_conf_ctrlmember), Toast.LENGTH_SHORT).show();
			return;
		}
		
		if (conference.getString("conference_accessmember", "").length() == 0){
			Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_no_conf_accessmember), Toast.LENGTH_SHORT).show();
			return;
		}
		
		if (conference.getString("conference_starttime", "").length() == 0){
			Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_no_conf_starttime), Toast.LENGTH_SHORT).show();
			return;			
		}
		
		if (conference.getString("conference_endtime", "").length() == 0){
			Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_no_conf_enddtime), Toast.LENGTH_SHORT).show();
			return;			
		}
		if (checkTime() == false){
			return;
		...
		if (conference.getString("conference_endtime", "").compareTo(conference.getString("conference_starttime", "")) < 0){
			Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_no_conf_vilid_time), Toast.LENGTH_SHORT).show();
			return;			
		}
		...
		String meetNoZd = conference.getString("meetNoZd", "");
		String meetNo_ = conference.getString("inmeetno", "");
		/*
		...
		*	
		*/
		String url ="MeetingManage/mobile/updateBasicInfo.action?typeId="+typeId+"&title="+title+"&address="+address+"&" +
				"startdate="+startdate+"&EndDate="+EndDate+"&managerId="+managerId+"&remark="+remark+"&nodifyType="+nodifyType+"&" +
				"meetNo="+meetNo_+"&managerId2="+managerId2 + "&meetId=" + meetId + "&meetNoZd=" + meetNoZd;
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler(){
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_network), Toast.LENGTH_SHORT).show();
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("用户未登陆")){
					Toast.makeText(ConChangeTwoActivity.this, getString(R.string.error_login), Toast.LENGTH_SHORT).show();
					return;
				}	
				
				if (response.contains("success")){
					Toast.makeText(ConChangeTwoActivity.this, "变更成功", Toast.LENGTH_SHORT).show();

				} else {
					try {
						JSONObject jso = new JSONObject(response);
						String msg = jso.getString("msg");
						Toast.makeText(ConChangeTwoActivity.this, msg, Toast.LENGTH_SHORT).show();
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
						Toast.makeText(ConChangeTwoActivity.this, "变更失败", Toast.LENGTH_SHORT).show();
					}
				}
			}
		});	
