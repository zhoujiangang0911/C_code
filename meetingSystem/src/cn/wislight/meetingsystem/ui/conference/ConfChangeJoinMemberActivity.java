...
public class ConfChangeJoinMemberActivity extends BaseActivity implements
		OnClickListener {
	...
	private void getMeetInfo() {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(ConfChangeJoinMemberActivity.this,
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("用户未登陆")){
					Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_login), Toast.LENGTH_SHORT).show();
					return;
				}	
				
				JSONArray joinmember_list;
				try {
					joinmember_list = new JSONObject(response).getJSONArray("meetpersonlist");
	...
	private void searchMyCreatedTempPerson() {
		...
		MeetingSystemClient.get(url, null, new AsyncHttpResponseHandler(){
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				loadingdiag.hide();
				Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_network), Toast.LENGTH_SHORT).show();
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				try {
					...
					e.printStackTrace();
					Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_dataabout), Toast.LENGTH_SHORT).show();
	...
	public void setupView() {
		setContentView(R.layout.conference_change_joinmember);
	}
    ...
    private void deleteMeetPersonTmp() {
    	...
    	if (length == 0){
    			Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_no_selected_attender), Toast.LENGTH_SHORT).show();
    		return;
    	}
		...
		for (int i=0; i<length; i++){
			...
			if (i == length-1 ){
				conference_joinmember += ele.getId();                       			
			} else {
				conference_joinmember += ele.getId()+",";  	        			     			
			}
		}        		
		...
		String url="MeetingManage/mobile/deleteMeetPersonTmp.action";
		RequestParams params = new RequestParams();
		params.put("type", 1);
		params.put("info", conference_joinmember);
		...
		MeetingSystemClient.post(url, params, new AsyncHttpResponseHandler()
		{
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				if (response.contains("0")){
					Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.conf_member_change_success), Toast.LENGTH_SHORT).show();
					
					getMeetInfo();
					
				} else{
					Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.conf_member_change_fail), Toast.LENGTH_SHORT).show();
				}
			}
		});	
	}
	...
	private void AddToHasSel() {
    	...
    	if (length == 0){
    		if (bDuplicated){
    			Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_person_duplicted), Toast.LENGTH_SHORT).show();
    		}else{
    			Toast.makeText(ConfChangeJoinMemberActivity.this, getString(R.string.error_no_person_to_add), Toast.LENGTH_SHORT).show();	
    		}
    		return;
    	}
		String conference_joinmember = "";     
		...
		String url="MeetingManage/mobile/insetRecordTmp.action";
		RequestParams params = new RequestParams();
