 ...
 *
 */
public class ServantMainPageWaitToHandleDetailActivity extends BaseActivity implements OnClickListener{
	...
	private Button btnGovermentaffairTransfer;
	
	private LinearLayout govermentaffairAcceptLayout;
	private LinearLayout govermentaffairTransferLayout;
	...
	public void setUpView() {
		setContentView(R.layout.activity_servant_mainpage_waitto_handle_detail);
		
		govermentaffairAcceptLayout = (LinearLayout) findViewById(R.id.govermentaffairAcceptLayout);
		govermentaffairTransferLayout = (LinearLayout) findViewById(R.id.govermentaffairTransferLayout);
		...
		btnGovermentaffairTransfer = (Button) findViewById(R.id.govermentaffairTransfer);
				
		txtBusinessaffairAddress.setOnClickListener(this);		
		btnGovermentaffairAccept.setOnClickListener(this);
		btnGovermentaffairTransfer.setOnClickListener(this);
	...
	public void fillData(String govermentAffairId){		
		...
		PublicServiceClient.get(url, null, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int arg0, Header[] arg1, byte[] body,
					Throwable error) {
				// loadingdiag.hide();
				Toast.makeText(ServantMainPageWaitToHandleDetailActivity.this,
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				try {
					...
					if (result.equals("success")){
						...
						if(state.equals(GovermentAffair.STATE.DAIJIESHOU)){
							txtGovermentaffairState.setText("待接受");
							govermentaffairAcceptLayout.setVisibility(View.VISIBLE);
							govermentaffairTransferLayout.setVisibility(View.VISIBLE);
						} else {
							txtGovermentaffairState.setText("");
						}											
					}else {
						Toast.makeText(ServantMainPageWaitToHandleDetailActivity.this,"初始化失败", Toast.LENGTH_SHORT)
								.show();
					}
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				try {
					...
					if (jsonArray.length() == 0) {
						...
						txtGovermentAffairProcedureLog.setText(log);
					}
				} catch (JSONException e) {
					e.printStackTrace();
					// loadingdiag.hide();
	...
	public void onClick(View v) {
		...
		case R.id.govermentaffairTransfer:
			Intent intentTransfer = new Intent(this, ServantGovermentAffairTransferActivity.class);
			intentTransfer.putExtra("govermentAffairId", govermentAffairId);
		...
		PublicServiceClient.get(url, null, new AsyncHttpResponseHandler() {		
			...
			public void onSuccess(int arg0, Header[] arg1, byte[] body) {
				...
				try {
					...
					String result = jsonObject.getString("result");
					if (result.equals("success")){
						Toast.makeText(ServantMainPageWaitToHandleDetailActivity.this,"接受事务成功", Toast.LENGTH_SHORT)
						.show();											
					}else {
						Toast.makeText(ServantMainPageWaitToHandleDetailActivity.this,"接受事务失败", Toast.LENGTH_SHORT)
								.show();
					}
					finish();
