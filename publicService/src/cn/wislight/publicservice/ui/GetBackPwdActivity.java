...
public class GetBackPwdActivity extends BaseActivity implements OnClickListener{
	...
	private void getVerifyCodeByLoginname(String loginname){
		...
		PublicServiceClient.get(url, null, new AsyncHttpResponseHandler() {
			...
			public void onFailure(int statusCode, Header[] arg1, byte[] body,
					Throwable error) {
				//loadingdiag.hide();
				Toast.makeText(GetBackPwdActivity.this,
			...
			public void onSuccess(int stautsCode, Header[] arg1, byte[] body) {
				...
				try {
					...
					if (result.equals("success")) {
						Toast.makeText(GetBackPwdActivity.this, "验证码已发送，请注意查收",
								100).show();						
					} else{
						Toast.makeText(GetBackPwdActivity.this, "验证码发送失败",
								100).show();
					}
	...
	private void changePasswordByLoginname(){
		...
		PublicServiceClient.post(url, params, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int stautsCode, Header[] arg1, byte[] body) {
				...
				try {
					...
					String result = jsonObject.getString("result");
					if (result.equals("success")) {
						Toast.makeText(GetBackPwdActivity.this, "修改成功",
								100).show();
						finish();					
					} else{
						Toast.makeText(GetBackPwdActivity.this, "修改失败",
								100).show();
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
